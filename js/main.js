...
function validateMyForm(e){
    ...
    .catch(()=>{
     ...
     document.getElementById('message').innerHTML=" Your Email Doesnot exist";
...
function finalsubmit(){
    ...
    var scondformfinal = document.getElementById("secondformfinal");
    scondformfinal.style.display="block";
    
    var finalsubmit = document.getElementById("finalsubmit");
    finalsubmit.style.display="none";
    
    var partmail = document.getElementById('email_final').value;
    console.log(partmail);
    var alreadyRegister = checkIfAlreadyRegister(allUser, partmail);
    console.log(alreadyRegister);
    if (alreadyRegister) {
        currentUser = alreadyRegister;
        const userlocal = localStorage.setItem('currentUser', JSON.stringify(currentUser));
        console.log('allready sdkk+++++++++', alreadyRegister);
         document.getElementById('email_second_final').value= alreadyRegister.email;
         console.log(alreadyRegister.email);
         console.log(alreadyRegister.name);
         console.log(part_type);
         console.log(organisation_type);
        
         document.getElementById('final_name').value= alreadyRegister.name;
         document.getElementById('part_type').value= alreadyRegister.participantType;
         document.getElementById('organisation_type').value= alreadyRegister.organization;
        
        
    } else {
        
         var scondformfinal = document.getElementById("secondformfinal");
    scondformfinal.style.display="none";
        
          var finalsubmit = document.getElementById("finalsubmit");
    finalsubmit.style.display="block";
        
      document.getElementById('message').innerHTML=" Email doesn't exits";  
        setTimeout(vanish, 3000);
...
function checkIfAlreadyRegister(arr, name) {
    var keys = Object.keys(arr);
    for(var i=0; i< keys.length; i++){
        var k = arr[keys[i]].email;
        if (name === k ) {
...
function secondfinalsubmit(){
    
    const name = document.getElementById('final_name').value;
    const email = document.getElementById('email_second_final').value;
    const organization = document.getElementById('organisation_type').value;
    const participant = document.getElementById('part_type').value;
    const part_design = document.getElementById('Designation').value;
    const organ_name = document.getElementById('organisation_name').value;
   ...
   firebase.database().ref('final').push({
        name:name,
        email: email,
        organization: organization,
        participantType: participant,
        organizationName: organ_name,
        Designation: part_design   
    })
    .then(()=>{
        document.getElementById("secondformdrdo").style.visibility = "hidden";
       document.getElementById("descrip_heading").style.visibility = "hidden";
        setTimeout(finalregistrationmsg, 1000);
    ...
    function finalregistrationmsg(){
    document.getElementById('messagemake').innerHTML="Thanks for submitting the details. Please check your inbox for confirmation mail"; 
    }
}
